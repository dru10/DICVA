...
def get_mel_spec_tf(audio, fs, n_mels=64, n_fft=255, hop_len=128):
...
def update_supported_commands():
    read_file = 'mydata.data'
    fd = open(read_file, 'rb')
    return pickle.load(fd)

def store_commands(commands):
    outfile = 'mydata.data'
    fw = open(outfile, 'wb')
    pickle.dump(commands, fw)
    fw.close()
